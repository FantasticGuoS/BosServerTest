...
public class IDCardUtil {
	...
	// wi =2(n-1)(mod 11) 前17位的权重
	final int[] wi = { 7, 9, 10, 5, 8, 4, 2, 1, 6, 3, 7, 9, 10, 5, 8, 4, 2, 1 };
	// verify digit 余数所对应的校验码
	 ...
	 * @return
	 */
	public boolean verifyLength(String cardid) {
	 ...
	 * @return
	 */
	public boolean verifyAreaCode(String cardid) {
	 ...
	 * @return
	 */
	public boolean verifyBirthdayCode(String cardid) {
		// 验证月份
		String month = cardid.substring(10, 12);
		boolean isEighteenCode = (18 == cardid.length());
		...
		// 验证日期
		String dayCode = cardid.substring(12, 14);
		Integer day = dateMap.get(month);
		String yearCode = cardid.substring(6, 10);
	 ...
	 * @return
	 */
	public boolean containsAllNumber(String cardid) {
		String str = "";
		if (cardid.length() == 15) {
	 ...
	 * @return
	 */
	public boolean verify(String cardid) {
		_codeError = "";
		// 验证身份证位数,15位和18位身份证
		if (!verifyLength(cardid)) {
			return false;
		}
		// 验证身份除了最后位其他的是否包含字母
		if (!containsAllNumber(cardid)) {
			return false;
		}
		...
		String eifhteencard = "";
		if (cardid.length() == 15) {
			eifhteencard = updateEighteen(cardid);
		} else {
			eifhteencard = cardid;
		}
		// 验证身份证的地区码
	 ...
	 * @return
	 */
	public boolean verifyMOD(String cardid) {
		String verify = cardid.substring(17, 18);
		if ("x".equals(verify)) {
			cardid = cardid.replaceAll("x", "X");
			verify = "X";
		}
		String verifyIndex = getVerify(cardid);
	 ...
	 * @return
	 */
	public String getVerify(String cardid) {
		int remaining = 0;
	 ...
	 * @return
	 */
	public String updateEighteen(String fifteencardid) {
